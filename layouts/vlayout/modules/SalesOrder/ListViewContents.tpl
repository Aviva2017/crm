...
<div class="listViewEntriesDiv contents-bottomscroll">
	...
	<table class="table table-bordered listViewEntriesTable">
        ...
        <tr>
			...
			{/foreach}
			<td>
				<button class="btn" data-trigger="listSearch"><i class="icon icon-search"></i></button>
			</td>
        </tr>
        {/if}
		...
		<tr class="listViewEntries" data-id='{$LISTVIEW_ENTRY->getId()}' data-recordUrl='{$LISTVIEW_ENTRY->getDetailViewUrl()}' id="{$MODULE}_listView_row_{$smarty.foreach.listview.index+1}">
            <td  width="10" class="{$WIDTHTYPE}">
				<input type="checkbox" value="{$LISTVIEW_ENTRY->getId()}" class="listViewEntriesCheckBox"/>
			</td>
			<td width="10" class="minAction">
                {if $IS_MODULE_EDITABLE && $LISTVIEW_ENTRY->CHECKABLE}
					<a href="javascript:void(0);" class="checkBtn"><i class="icon-ok-sign alignMiddle"></i></a>
				{/if}
			</td>
			{foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS}
